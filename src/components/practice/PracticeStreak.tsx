...
export function PracticeStreak({ currentStreak, longestStreak }: PracticeStreakProps) {
  ...
  return (
    <motion.div
      ...
      className="card-elevated px-4 py-3 space-y-2.5"
    >
      ...
      {nextMilestone && (
        <div className="space-y-1">
          ...
          <p className="text-[9px] text-muted-foreground text-right tabular-nums">
            {currentStreak}/{nextMilestone} days to <span className="font-semibold text-foreground">
              {nextMilestone === 7 ? "Weekly Warrior 🏅" : nextMilestone === 14 ? "Two-Week Titan 🏅" : "30-Day Legend 🏆"}
            </span>
          </p>
        </div>
      )}
